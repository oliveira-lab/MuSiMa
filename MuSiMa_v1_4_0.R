...
for (motif in motifs) {
  ...
  # Set gap degrees: first gap is 15 degrees, others are 5 degrees
num_sectors            <- nrow(df)
...
circos.par(
  ...
  col_fun <- colorRamp2(c(-5, -2.5, 0, 2.5, 5), c("blue", "navy", "green", "red", "darkred"))
  ...
  if (palindromic) {
    ...
    for (i in seq_along(window_sizes)) {
      ...
      if (nrow(plot_data) == 0) {
        cat(sprintf("Warning: Palindromic sense track %d has no data\n", i))
        next
      }
      ...
      cat(sprintf("Plotted palindromic sense track %d\n", i))
    }
  } else {
    # Plot sense tracks with legends
    results_list_sense <- motif_results_list[["sense"]]
    if (length(results_list_sense) != length(window_sizes)) {
      stop(sprintf("Error: Expected %d sense tracks, but found %d", length(window_sizes), length(results_list_sense)))
    }
    for (i in seq_along(window_sizes)) {
      plot_data <- results_list_sense[[i]][, c("chrom", "start", "end", "z_score")]
      if (nrow(plot_data) == 0) {
        cat(sprintf("Warning: Sense track %d has no data\n", i))
        next
      }
      window_size_label <- window_sizes[i]
      ...
      cat(sprintf("Plotted sense track %d\n", i))
    }
    # Plot antisense tracks with legends
    results_list_antisense <- motif_results_list[["antisense"]]
    if (length(results_list_antisense) != length(window_sizes)) {
      stop(sprintf("Error: Expected %d antisense tracks, but found %d", length(window_sizes), length(results_list_antisense)))
    }
    for (i in seq_along(window_sizes)) {
      plot_data <- results_list_antisense[[i]][, c("chrom", "start", "end", "z_score")]
      if (nrow(plot_data) == 0) {
        cat(sprintf("Warning: Antisense track %d has no data\n", i))
        next
      }
      window_size_label <- window_sizes[i]
      ...
      cat(sprintf("Plotted antisense track %d\n", i))
    }
  }
  
  # Add legend
  draw(Legend(col_fun = col_fun, title = "Motif Z-Score", at = c(-5, -2.5, 0, 2.5, 5), direction = "vertical"), 
  ...
  for (mt in motifs_to_process) {
    ...
    for (i in seq_along(window_sizes)) {
      cat(sprintf("\n=== Strand: %s, Window Size: %d ===\n", strand, window_sizes[i]), file = file_conn)
